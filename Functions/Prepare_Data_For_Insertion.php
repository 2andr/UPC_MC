<?php
include UPCP_CD_PLUGIN_PATH . 'makers_addon/Prepare_Maker_Insertion.php';
...
function Add_Edit_Product() {
	...
	if (isset($_POST['Makers'])) {$Makers = $_POST['Makers'];}
	else {$Makers = null;}
	if (isset($_POST['Profuses'])) {$Profuses = $_POST['Profuses'];}
	else {$Profuses = null;}
	...
	if (!isset($error) or $error == __('No file was uploaded.', 'ultimate-product-catalogue')) {
			...
			/* Pass the data to the appropriate function in Update_Admin_Databases.php to edit the product */
					...
					$user_update = Edit_UPCP_Product($Item_ID, $Item_Name, $Item_Slug, $Item_Photo_URL, $Item_Description, $Item_Price, $Item_Sale_Price, $Item_Sale_Mode, $Item_SEO_Description, $Item_Link, $Item_Display_Status, $Category_ID, $Global_Item_ID, $Item_Special_Attr, $SubCategory_ID, $Tags, $Makers, $Profuses, $Related_Products, $Next_Previous, $Skip_Nonce, $Item_WC_ID);
			}
