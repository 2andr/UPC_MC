<?php
include UPCP_CD_PLUGIN_PATH . 'makers_addon/Update_Admin_Databases_Makers.php';
...
function Add_UPCP_Product($Item_Name, $Item_Slug, $Item_Photo_URL, $Item_Description, $Item_Price, $Item_Sale_Price, $Item_Sale_Mode, $Item_SEO_Description, $Item_Link, $Item_Display_Status = "", $Category_ID = "", $Global_Item_ID = "", $Item_Special_Attr = "", $SubCategory_ID = "", $Tags = array(), $Makers = array(), $Profuses = array(), $Related_Products = "", $Next_Previous = "", $Skip_Nonce = "No", $WC_ID = 0, $WC_Update = "No") {
		...
		// Create the makers item in the makers items table for each "maker" checkbox
		// and update the Item_Count column in the makers table in the database
		$_GET['Item_ID'] = $Item_ID = $wpdb->insert_id;

		foreach ($Makers as $Maker) {
				$wpdb->insert($makers_meta_table_name,
								array('Maker_ID' => $Maker,
											'Item_ID' => $Item_ID)
				);
				$wpdb->query("UPDATE $makers_table_name SET Maker_Item_Count=Maker_Item_Count + 1 WHERE Maker_ID =" . $Maker);
		}

		// Create the profuses item in the profuses items table for each "profuse" checkbox
		// and update the Item_Count column in the profuses table in the database
		$_GET['Item_ID'] = $Item_ID = $wpdb->insert_id;
		foreach ($Profuses as $Profuse) {
				$wpdb->insert($profuses_meta_table_name,
								array('Profuse_ID' => $Profuse,
											'Item_ID' => $Item_ID)
				);
				$wpdb->query("UPDATE $profuses_table_name SET Profuse_Item_Count=Profuse_Item_Count + 1 WHERE Profuse_ID =" . $Profuse);
		}
...
function Edit_UPCP_Product($Item_ID, $Item_Name, $Item_Slug, $Item_Photo_URL, $Item_Description, $Item_Price, $Item_Sale_Price, $Item_Sale_Mode, $Item_SEO_Description, $Item_Link, $Item_Display_Status = "", $Category_ID = "", $Global_Item_ID = "", $Item_Special_Attr = "", $SubCategory_ID = "", $Tags = array(), $Makers = array(), $Profuses = array(), $Related_Products = "", $Next_Previous = "", $Skip_Nonce = "No", $Item_WC_ID = 0, $WC_Update = "No") {
		...
		// Delete the maker item in the maker items table for the given Item_ID
		// and update the Item_Count column in the makers table in the database
		$Maker_Items = $wpdb->get_results("SELECT Maker_ID FROM $makers_meta_table_name WHERE Item_ID='" . $Item_ID ."'");
		foreach ($Maker_Items as $Maker_Item) {
				$wpdb->query("UPDATE $makers_table_name SET Maker_Item_Count=Maker_Item_Count - 1 WHERE Maker_ID =" . $Maker_Item->Maker_ID);
		}
		$wpdb->delete(
		...
		// Delete the profuse item in the profuse items table for the given Item_ID
		// and update the Item_Count column in the profuses table in the database
		$Profuse_Items = $wpdb->get_results("SELECT Profuse_ID FROM $profuses_meta_table_name WHERE Item_ID='" . $Item_ID ."'");
		foreach ($Profuse_Items as $Profuse_Item) {
				$wpdb->query("UPDATE $profuses_table_name SET Profuse_Item_Count=Profuse_Item_Count - 1 WHERE Profuse_ID =" . $Profuse_Item->Profuse_ID);
		}
		$wpdb->delete(
				$profuses_meta_table_name,
				array('Item_ID' => $Item_ID)
		);
		...
		//$_GET['Item_ID'] = $Item_ID = $wpdb->insert_id;
		foreach ($Makers as $Maker) {
				$wpdb->insert($makers_meta_table_name, array('Maker_ID' => $Maker, 'Item_ID' => $_GET['Item_ID']));
				$wpdb->query("UPDATE $makers_table_name SET Maker_Item_Count=Maker_Item_Count + 1 WHERE Maker_ID =" . $Maker);
		}
...
function Update_UPCP_Options() {
	...
	$Sidebar_Items_Order = Update_UPCP_Options_Makers(); 
