<?php
include UPCP_CD_PLUGIN_PATH . 'makers_addon/Shortcodes_Makers.php';
...
function Insert_Product_Catalog($atts) {
	...
	global $wpdb, $categories_table_name, $subcategories_table_name, $tags_table_name, $tagged_items_table_name, $tag_groups_table_name, $catalogues_table_name, $catalogue_items_table_name, $items_table_name, $fields_table_name, $fields_meta_table_name, $makes_table_name, $makers_meta_table_name, $profuses_table_name, $profuses_meta_table_name, $item_videos_table_name;
	...
	if (isset($_GET['Makers'])) {$makers = explode(",", $_GET['Makers']);}
	elseif (isset($_POST['Makers']) and $_POST['Makers'] != "") {$makers = explode(",", $_POST['Makers']);}
	elseif ($makers == "") {$makers = array();}
	else {$makers = explode(",", $makers);}

	if (isset($_GET['Profuses'])) {$profuses = explode(",", $_GET['Profuses']);}
	elseif (isset($_POST['Profuses']) and $_POST['Profuses'] != "") {$profuses = explode(",", $_POST['Profuses']);}
	elseif ($profuses == "") {$profuses = array();}
	else {$profuses = explode(",", $profuses);}
	...
	if ($category != "" or $subcategory != "" or $tags != "" or $prod_name != "" or $makers !="" or $profuses !="") {$Filtered = "Yes";}
	else {$Filtered = "No";}
	...
	foreach ($CatalogueItems as $CatalogueItem) {
		...
		if ($CatalogueItem->Item_ID != "" and $CatalogueItem->Item_ID != 0) {
			...
			$ProdMakerObj = $wpdb->get_results("SELECT Maker_ID FROM $makers_meta_table_name WHERE Item_ID=" . $Product->Get_Item_ID());
			$ProdProfuseObj = $wpdb->get_results("SELECT Profuse_ID FROM $profuses_meta_table_name WHERE Item_ID=" . $Product->Get_Item_ID());
			...
			if ($Product->Get_Field_Value('Item_Display_Status') != "Hide") {
				...
				if (sizeOf($category) == 0 or in_array($Product->Get_Field_Value('Category_ID'), $category)) {
					if (sizeOf($subcategory) == 0 or in_array($Product->Get_Field_Value('SubCategory_ID'), $subcategory)) {
						$Maker_Check = CheckMakers($makers, $ProdMakerObj);
						if ($Maker_Check == "Yes") {
							$Profuse_Check = CheckProfuses($profuses, $ProdProfuseObj);
							if ($Profuse_Check == "Yes") {
				...
				if ($ajax_reload == "No") {FilterCount($Product, $ProdTagObj, $Prod_Custom_Fields, $ProdMakerObj, $ProdProfuseObj);}
			}
	...
	foreach ($ProdMakers as $key=>$value) {$ProdMakersString .= $key . ",";}
	$ProdMakersString = trim($ProdMakersString, " ,");
	foreach ($ProdProfuses as $key=>$value) {$ProdProfusesString .= $key . ",";}
	$ProdProfusesString = trim($ProdProfusesString, " ,");
...
function UPCP_AddProduct($format, $Product_Object, $Tags, $AjaxReload = "No", $AjaxURL = "") {
	...
	global $wpdb, $categories_table_name, $subcategories_table_name, $tags_table_name, $tagged_items_table_name, $makers_table_name, $makers_meta_table_name, $profuses_table_name, $profuses_meta_table_name, $catalogues_table_name, $catalogue_items_table_name, $items_table_name, $item_images_table_name, $item_videos_table_name;
	global $ProdCats, $ProdSubCats, $ProdTags, $ProdMakers, $ProdMProfuses, $ProdCustomFields, $link_base, $UPCP_Options;
	...
	if ($format == "Thumbnail") {
		...
		if (is_array($thumbnailTags)) {
			...
			foreach ($thumbnailTags as $thumbnailTag) {
				...
				if (is_object($thumbnailTagInfo)) $thumbnailTagsString .= $thumbnailTagInfo->Tag_Name . ", ";
			}
		}
		...
		$thumbnailMakers = $wpdb->get_results("SELECT Maker_ID FROM $makers_meta_table_name WHERE Item_ID=" . $Product_Object->Get_Item_ID());
		if (is_array($thumbnailMakers)) {
			$thumbnailMakersString = '';
			foreach ($thumbnailMakers as $thumbnailMaker) {
				$MakerInfo = $wpdb->get_row("SELECT Maker_Name FROM $makers_table_name WHERE Maker_ID=" . $thumbnailMaker->Maker_ID);
				$thumbnailMakersString .= $MakerInfo->Maker_Name . ", "; 
			}
		}
		if (isset($thumbnailMakersString)) {$thumbnailMakersString = trim($thumbnailMakersString, " ,");}
		if($Display_Makers_In_Thumbnails == 'Yes' && $thumbnailMakersString != ''){
			$ProductString .= "<div class='prod-cat-display-categories-tags upcp-thumb-display-tags'>";
				$ProductString .= "<span class='upcp-display-makers-label'>" .	$MakersCard_Label . "</span>";
				$ProductString .= $thumbnailMakersString;
			$ProductString .= "</div>\n";
		}

		$thumbnailProfuses = $wpdb->get_results("SELECT Profuse_ID FROM $profuses_meta_table_name WHERE Item_ID=" . $Product_Object->Get_Item_ID());
		if (is_array($thumbnailProfuses)) {
			$thumbnailProfusesString = '';
			foreach ($thumbnailProfuses as $thumbnailProfuse) {
				$ProfuseInfo = $wpdb->get_row("SELECT Profuse_Name FROM $profuses_table_name WHERE Profuse_ID=" . $thumbnailProfuse->Profuse_ID);
				$thumbnailProfusesString .= $ProfuseInfo->Profuse_Name . ", ";
			}
		}
		if (isset($thumbnailProfusesString)) {$thumbnailProfusesString = trim($thumbnailProfusesString, " ,");}
		if($Display_Profuses_In_Thumbnails == 'Yes' && $thumbnailProfusesString != ''){
			$ProductString .= "<div class='prod-cat-display-categories-tags upcp-thumb-display-tags'>";
				$ProductString .= "<span class='upcp-display-profuses-label'>" .	$ProfusesCard_Label . "</span>";
				$ProductString .= $thumbnailProfusesString;
			$ProductString .= "</div>\n";
		}
...
function SingleProductPage() {
	global $wpdb, $items_table_name, $item_images_table_name, $fields_table_name, $fields_meta_table_name, $makers_table_name, $makers_meta_table_name, $profuses_table_name, $profuses_meta_table_name, $tagged_items_table_name, $tags_table_name, $tag_groups_table_name, $item_videos_table_name;
	global $link_base;
	global $UPCP_Options;
	...
	//Create the maker string for filtering
	$MakersNameString = "";
	$Makers = $wpdb->get_results("SELECT Maker_ID FROM $makers_meta_table_name WHERE Item_ID=" . $Product->Item_ID);
	if (is_array($Makers)) {
		foreach ($Makers as $Maker) {
			$MakerInfo = $wpdb->get_row("SELECT Maker_Name FROM $makers_table_name WHERE Maker_ID=" . $Maker->Maker_ID);
			$MakersNameString .= $MakerInfo->Maker_Name . ", ";
		}
	}
	$MakersNameString = trim($MakersNameString, " ,");
	
	//Create the profuse string for filtering
	$ProfusesNameString = "";
	$Profuses = $wpdb->get_results("SELECT Profuse_ID FROM $profuses_meta_table_name WHERE Item_ID=" . $Product->Item_ID);
	if (is_array($Profuses)) {
		foreach ($Profuses as $Profuse) {
			...
			$ProfusesNameString .= $ProfuseInfo->Profuse_Name . ", ";
		}
	}
	$ProfusesNameString = trim($ProfusesNameString, " ,");
	...
	elseif ($Custom_Product_Page == "Tabbed" || $Custom_Product_Page == "Shop_Style") {
		...
		$ProductString .= "<h2 class='upcp-tabbed-product-name'><span itemprop='name'>" . apply_filters('upcp_product_page_title', $Product->Item_Name, array('Item_ID' => $Product->Item_ID, 'Item_Title' => $Product->Item_Name)) . "</span> <span itemprop='maker'>( ". $MakersNameString. " )</span></h2>";
...
function BuildSidebar($category, $subcategory, $tags, $prod_name) {
	global $wpdb, $Full_Version, $ProdCats, $ProdSubCats, $ProdTags, $ProdCustomFields, $ProdMakers, $ProdProfuses, $ProdCatString, $ProdSubCatString, $ProdTagString, $ProdCustomFieldsString, $ProdMakersString, $ProdProfusesString, $Max_Price_Product, $Min_Price_Product;
	global $categories_table_name, $subcategories_table_name, $tags_table_name, $fields_table_name, $makers_table_name, $profuses_table_name;
	global $UPCP_Options;
...
//Makers addon start 
	...
	// Get the makers that apply to the products in the catalog
	if ($ProdMakersString != "") {$Makers = $wpdb->get_results("SELECT Maker_ID, Maker_Name, Maker_Image FROM $makers_table_name WHERE Maker_ID IN (" . $ProdMakersString . ") ORDER BY Maker_Name");} 
	else {$Makers = array();}
	...
	// Get the profuses that apply to the products in the catalog
	if ($ProdProfusesString != "") {$Profuses = $wpdb->get_results("SELECT Profuse_ID, Profuse_Name, Profuse_Description, Profuse_Image FROM $profuses_table_name WHERE Profuse_ID IN (" . $ProdProfusesString . ") ORDER BY Profuse_Name");} 
	else {$Profuses = array();}
...
//Makers addon end
	...
	foreach ($Sidebar_Items_Order as $Sidebar_Item) {
		...
		// Create the Makers checkboxes
		if ($Sidebar_Item == "Makers" and sizeof($Makers) > 0) {
			$MakersString = "<div id='prod-cat-sidebar-maker-div-" . $id . "' class='prod-cat-sidebar-maker-div'>\n";
			$MakersString .= "<div id='prod-cat-sidebar-maker-title-" . $id . "' class='prod-cat-sidebar-cat-title prod-cat-sidebar-maker-title prod-cat-sidebar-hover-"  . $Sidebar_Title_Hover . "' data-title='3'";
			if ($Sidebar_Title_Collapse == "yes") {$MakersString .= "onclick='UPCP_Show_Hide_Sidebar(this);'";}
			$MakersString .= "><h3>" . $Makers_Label . "</h3></div>\n";
			$MakersString .= "<div class='prod-cat-sidebar-content prod-cat-sidebar-content-maker' data-title='3' " . $Sidebar_Start_Collapsed_HTML . ">\n";
			if ($Makers_Control_Type == "Dropdown") {
				$MakersString .= "<select name='Makers' onchange='UPCP_DisplayPage(\"1\");' class='upcp-jquery-makers-dropdown'>";
				$MakersString .= "<option value='All'>" . $Show_All_Label . "</option>";
			}
			foreach ($Makers as $Maker) {
				if ($Makers_Control_Type == "Dropdown") {
					$MakersString .= "<option value='" . $Maker->Maker_ID . "' ";
					if (in_array($Maker->Maker_ID, $Makers)) {$MakersString .= "selected=selected";}
					$MakersString .= ">" . $Maker->Maker_Name . "</option>";}
				else {
					$MakersString .= "<div id='prod-cat-sidebar-maker-item-" . $Maker->Maker_ID . "' class='prod-cat-sidebar-maker prod-sidebar-checkbox-" . $Sidebar_Checkbox_Style . " checkbox-color-" . "$Color";
					if (in_array($Maker->Maker_ID, $Makers)) {$MakersString .= " highlight" . $Color;}
					$MakersString .= "'>\n";
					$MakersString .= "<input type='" . strtolower($Makers_Control_Type) . "' name='Makers' value='" . $Maker->Maker_ID . "'  onclick='UPCP_DisplayPage(\"1\"); UPCPHighlight(this, \"" . $Color . "\");' id='maker-" . $Maker->Maker_ID . "' class='jquery-prod-maker-value'";
					if (in_array($Maker->Maker_ID, $Makers)) {$MakersString .= "checked=checked";}
					$MakersString .= "><label class='upcp-label' for='maker-". $Maker->Maker_ID ."'><span> " . $Maker->Maker_Name . "</span> <span>(" . $ProdMakers[$Maker->Maker_ID] . ")</span></label>\n";
					$MakersString .= "</div>";
				}
			}
			if ($Makers_Control_Type == "Dropdown") {$MakersString .= "</select>";}
			$MakersString .= "</div>\n</div>\n";
			$SidebarString .= apply_filters('upcp_sidebar_makers_div', $MakersString);
		}

		// Create the Profuses checkboxes
		if ($Sidebar_Item == "Profuses" and sizeof($Profuses) > 0) {
			$ProfusesString = "<div id='prod-cat-sidebar-profuse-div-" . $id . "' class='prod-cat-sidebar-profuse-div'>\n";
			$ProfusesString .= "<div id='prod-cat-sidebar-profuse-title-" . $id . "' class='prod-cat-sidebar-cat-title prod-cat-sidebar-profuse-title prod-cat-sidebar-hover-"  . $Sidebar_Title_Hover . "' data-title='3'";
			if ($Sidebar_Title_Collapse == "yes") {$ProfusesString .= "onclick='UPCP_Show_Hide_Sidebar(this);'";}
			$ProfusesString .= "><h3>" . $Profuses_Label . "</h3></div>\n";
			$ProfusesString .= "<div class='prod-cat-sidebar-content prod-cat-sidebar-content-profuse' data-title='3' " . $Sidebar_Start_Collapsed_HTML . ">\n";
			if ($Profuses_Control_Type == "Dropdown") {
				$ProfusesString .= "<select name='Profuses' onchange='UPCP_DisplayPage(\"1\");' class='upcp-jquery-profuses-dropdown'>";
				$ProfusesString .= "<option value='All'>" . $Show_All_Label . "</option>";
			}
			foreach ($Profuses as $Profuse) {
				if ($Profuses_Control_Type == "Dropdown") {
					$ProfusesString .= "<option value='" . $Profuse->Profuse_ID . "' ";
					if (in_array($Profuse->Profuse_ID, $Profuses)) {$ProfusesString .= "selected=selected";}
					$ProfusesString .= ">" . $Profuse->Profuse_Name . "</option>";}
				else {
					$ProfusesString .= "<div id='prod-cat-sidebar-profuse-item-" . $Profuse->Profuse_ID . "' class='prod-cat-sidebar-profuse prod-sidebar-checkbox-" . $Sidebar_Checkbox_Style . " checkbox-color-" . "$Color";
					if (in_array($Profuse->Profuse_ID, $Profuses)) {$ProfusesString .= " highlight" . $Color;}
					$ProfusesString .= "'>\n";
					$ProfusesString .= "<input type='" . strtolower($Profuses_Control_Type) . "' name='Profuses[]' value='" . $Profuse->Profuse_ID . "'  onclick='UPCP_DisplayPage(\"1\"); UPCPHighlight(this, \"" . $Color . "\");' id='profuse-" . $Profuse->Profuse_ID . "' class='jquery-prod-profuse-value'";
					if (in_array($Profuse->Profuse_ID, $Profuses)) {$ProfusesString .= "checked=checked";}
					$ProfusesString .= "><label class='upcp-label' for='profuse-". $Profuse->Profuse_ID ."'><span data-tooltip='" . $Profuse->Profuse_Description . "' data-tooltip-position='right' > " . $Profuse->Profuse_Name . "</span> <span>(" . $ProdProfuses[$Profuse->Profuse_ID] . ")</span></label>\n";
					$ProfusesString .= "</div>";
				}
			}
			if ($Profuses_Control_Type == "Dropdown") {$ProfusesString .= "</select>";}
			$ProfusesString .= "</div>\n</div>\n";
			$SidebarString .= apply_filters('upcp_sidebar_profuses_div', $ProfusesString);
			//test
		}
...
function UPCP_Product_Comparison($Products = array(), $omit_fields) {
	global $wpdb, $fields_table_name, $link_base, $item_images_table_name;
	global $UPCP_Options;
	...
	$SubCategories_Label = $UPCP_Options->Get_Option("UPCP_SubCategories_Label");
	$Tags_Label = $UPCP_Options->Get_Option("UPCP_Tags_Label");
	$Makers_Label = $UPCP_Options->Get_Option("UPCP_Makers_Label");
	$Profuses_Label = $UPCP_Options->Get_Option("UPCP_Profuses_Label");
	$Back_To_Catalogue_Label = $UPCP_Options->Get_Option("UPCP_Back_To_Catalogue_Label");
	...
	foreach ($Products as $Product_ID) {
		...
		if ( $Item_Image )
			$PhotoURL = $Item_Image->Item_Image_URL;
		...
		if (!in_array("Makers", $Omit_Fields_Array)) {
			$ReturnString .= "<div class='upcp-product-comparison-maker upcp-pc-field'>";
			$ReturnString .= "<div class='upcp-product-comparison-maker-label upcp-pc-label'>" . $Makers_Text . "</div>";
			$ReturnString .= "<div class='upcp-product-comparison-subcategory-value upcp-pc-value'>" . $Product->Get_ProductMaker() . "</div>";
			$ReturnString .= "</div>";
		}
		if (!in_array("Profuses", $Omit_Fields_Array)) {
			$ReturnString .= "<div class='upcp-product-comparison-profuse upcp-pc-field'>";
			$ReturnString .= "<div class='upcp-product-comparison-profuse-label upcp-pc-label'>" . $Profuses_Text . "</div>";
			$ReturnString .= "<div class='upcp-product-comparison-subcategory-value upcp-pc-value'>" . $Product->Get_ProductProfuse() . "</div>";
			$ReturnString .= "</div>";
		}
...
function FilterCount($Product, $Tags, $Custom_Fields, $Makers, $Profuses) {
	global $Product_Attributes, $ProdCats, $ProdSubCats, $ProdTags, $ProdCustomFields, $ProdMakers, $ProdProfuses;
	global $wpdb, $fields_table_name, $makers_table_name, $profuses_table_name;
	...
	if (is_array($Makers)) {
		foreach ($Makers as $Maker) {
			if(!array_key_exists($Maker->Maker_ID, $ProdMakers)){$ProdMakers[$Maker->Maker_ID] = 0;}
			if (is_object($Maker))
			{
				$ProdMakers[$Maker->Maker_ID]++;
				$Product_Values['Makers'][] = $Maker->Maker_ID;
	...
	if (is_array($Profuses)) {
		foreach ($Profuses as $Profuse) {
			if(!array_key_exists($Profuse->Profuse_ID, $ProdProfuses)){$ProdProfuses[$Profuse->Profuse_ID] = 0;}
			if (is_object($Profuse))
			{
				$ProdProfuses[$Profuse->Profuse_ID]++;
				$Product_Values['Profuses'][] = $Profuse->Profuse_ID;
	...
	if (!isset($Product_Values['Makers'])) {
		$Product_Values['Makers'] = array();
	}
	if (!isset($Product_Values['Profuses'])) {
		$Product_Values['Profuses'] = array();
	}
