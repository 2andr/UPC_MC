...
// makers addon start
	jQuery('.upcp-catalogue-link ').hover(
		function(){jQuery(this).children('.upcp-prod-desc-makers').fadeIn(400);},
		function(){jQuery(this).children('.upcp-prod-desc-makers').fadeOut(400);}
	);
	jQuery('.upcp-minimal-img-div').hover(
		function(){jQuery(this).children('.upcp-prod-desc-makers').fadeIn(400);},
		function(){jQuery(this).children('.upcp-prod-desc-makers').fadeOut(400);}
	...
	jQuery('.upcp-catalogue-link ').hover(
		function(){jQuery(this).children('.upcp-prod-desc-profuses').fadeIn(400);},
		function(){jQuery(this).children('.upcp-prod-desc-profuses').fadeOut(400);}
	);
	jQuery('.upcp-minimal-img-div').hover(
		function(){jQuery(this).children('.upcp-prod-desc-profuses').fadeIn(400);},
		function(){jQuery(this).children('.upcp-prod-desc-profuses').fadeOut(400);}
...
// makers addon start
	var MakerBoxValues = [];
	var ProfuseBoxValues = [];

	jQuery('.jquery-prod-maker-value').each(function() {if (jQuery(this).prop('checked')) {MakerBoxValues.push(jQuery(this).val());}});
	jQuery('.upcp-jquery-maker-dropdown').each(function() {if (jQuery(this).val() != "All") {MakerBoxValues.push(jQuery(this).val());}});
	jQuery('.jquery-prod-profuse-value').each(function() {if (jQuery(this).prop('checked')) {ProfuseBoxValues.push(jQuery(this).val());}});
	jQuery('.upcp-jquery-profuse-dropdown').each(function() {if (jQuery(this).val() != "All") {ProfuseBoxValues.push(jQuery(this).val());}});
	/*
	jQuery('.jquery-prod-maker-value').each(function() {if (jQuery(this).prop('checked')) {
		Array_Value = jQuery(this).parent().parent().data('makerid') + "=" + jQuery(this).val();
		MakerBoxValues.push(Array_Value);
	}});
	jQuery('.jquery-prod-maker-select').each(function() {if (jQuery(this).val() != "All") {
		Array_Value = jQuery(this).data('makerid') + "=" + jQuery(this).val();
		MakerBoxValues.push(Array_Value);
	}});
	jQuery('.jquery-prod-maker-select').each(function() {if (jQuery(this).val() != "All") {
		Array_Value = jQuery(this).data('makerid') + "=" + jQuery(this).val();
	...
	jQuery('.upcp-maker-slider').each(function() {
    	var Maker_ID = jQuery(this).data("makerid");

    	var Array_Value = Maker_ID + "=" + jQuery("#upcp-maker-"+Maker_ID).slider("values", 0);
    	MakerBoxValues.push(Array_Value);
    	var Array_Value = Maker_ID + "=" + jQuery("#upcp-maker-"+Maker_ID).slider("values", 1);
    	MakerBoxValues.push(Array_Value);
    });
	*/
// makers addon end	
...
// makers addon start change
	var data = 'id=' + id + '&sidebar=' + sidebar + '&start_layout=' + current_layout + '&excluded_layouts=' + excluded_layouts + '&ajax_url=' + base_url + '&current_page=' + current_page + '&products_per_page=' + products_per_page + '&default_search_text=' + default_search_text + '&ajax_reload=Yes' + '&Prod_Name=' + SelectedProdName + '&max_price=' + max_price + '&min_price=' + min_price + '&Category=' + CatValues + '&SubCategory=' + SubCatValues + '&Tags=' + TagBoxValues + '&Makers=' + MakerBoxValues + '&Profuses=' + ProfuseBoxValues + '&Custom_Fields=' + encodeURIComponent(CFBoxValues) + '&request_count=' + RequestCount + '&action=update_catalogue';
	...
	jQuery.post(ajaxurl, data, function(response) {
		...
		if (parsed_response.request_count == RequestCount) {
			...
			if (CatValues.length == 0 && SubCatValues.length == 0 && TagBoxValues.length == 0 && CFBoxValues.length == 0 && MakerBoxValues.length == 0 && ProfuseBoxValues.length == 0) {jQuery('.prod-cat-category-label').each(function() {jQuery(this).removeClass('Hide-Item');});}
			...
			addInquiryAndCartHandlers();
			UPCP_Infinite_Scroll();
			UPCP_Adjust_Sidebar_Counts(min_price, max_price, CatValues, SubCatValues, TagBoxValues, CFBoxValues, MakerBoxValues, ProfuseBoxValues);
			Filtering_Running = "No";
		}
	});
// makers addon end change
...
function clearAllFilteringSelections() {
	...
	jQuery('.jquery-prod-cat-value, .jquery-prod-sub-cat-value, .jquery-prod-tag-value, .jquery-prod-cf-value, .jquery-prod-maker-value, .jquery-prod-profuse-value').prop('checked', false).parent().removeClass('highlightBlack').removeClass('highlightBlue').removeClass('highlightGrey');
	jQuery('.upcp-jquery-cat-dropdown, .upcp-jquery-subcat-dropdown, .upcp-jquery-tags-dropdown, .jquery-prod-cf-select, .jquery-prod-maker-select, .jquery-prod-profuse-value').val('All');
	jQuery('.upcp-custom-field-slider').each(function() {
		var Field_ID = jQuery(this).data('fieldid');
	...
	jQuery('.upcp-maker-slider').each(function() {
		var Maker_ID = jQuery(this).data('fieldid');
		max_value_int = parseInt(window['max_'+Maker_ID]); 
		min_value_int = parseInt(window['min_'+Maker_ID]);
		jQuery("#upcp-maker-"+Maker_ID).slider("values", [min_value_int, max_value_int]);
	});
	jQuery('.upcp-profuse-slider').each(function() {
		var Profuse_ID = jQuery(this).data('fieldid');
		max_value_int = parseInt(window['max_'+Profuse_ID]); 
		min_value_int = parseInt(window['min_'+Profuse_ID]);
		jQuery("#upcp-maker-"+Profuse_ID).slider("values", [min_value_int, max_value_int]);
	});
...
function UPCP_Adjust_Sidebar_Counts(min_price, max_price, CatValues, SubCatValues, TagBoxValues, CFBoxValues, MakerBoxValues, ProfuseBoxValues) {
	...
	var Sidebar_Counts = {"Categories" : {}, 'SubCategories' : {}, 'Tags' : {},  'CustomFields' : {},  'Makers' : {},  'Profuses' : {}};
	jQuery(Decoded_Products).each(function(index, el) {
		...
		if (MakerBoxValues.length != 0 && jQuery.inArray(el.Maker, MakerBoxValues) == -1) {Missing = (Missing == '' ? 'Maker' : 'Mismatch');}
		if (ProfuseBoxValues.length != 0 && jQuery.inArray(el.Profuse, ProfuseBoxValues) == -1) {Missing = (Missing == '' ? 'Profuse' : 'Mismatch');}
		...
		if (Missing != 'Mismatch') {
			...
			if (Missing == '' || Missing == 'Maker') {
				if (Sidebar_Counts['Makers'][el.Maker] === undefined) {Sidebar_Counts['Makers'][el.Maker] = 1;}
				else {Sidebar_Counts['Makers'][el.Maker] = Sidebar_Counts['Makers'][el.Maker] + 1;}
			}
			if (Missing == '' || Missing == 'Profuse') {
				if (Sidebar_Counts['Profuses'][el.Profuse] === undefined) {Sidebar_Counts['Profuses'][el.Profuse] = 1;}
				else {Sidebar_Counts['Profuses'][el.Profuse] = Sidebar_Counts['Profuses'][el.Profuse] + 1;}
			}
		}
	});
	...
	jQuery('.prod-cat-sidebar-maker label').each(function(index, el) {
		var ID = jQuery(this).attr('for').substring(4);
		if (Sidebar_Counts['Makers'][ID] === undefined) {
			jQuery(this).find('span span').html('(0)');
			if (ajax_translations.hide_empty == "Yes") {jQuery(this).parent().css('display', 'none');}
		}
		else {jQuery(this).parent().css('display', 'block').find('span span').html('(' + Sidebar_Counts['Makers'][ID] + ')');}
	});
	jQuery('.prod-cat-sidebar-profuse label').each(function(index, el) {
		var ID = jQuery(this).attr('for').substring(4);
		if (Sidebar_Counts['Profuses'][ID] === undefined) {
			jQuery(this).find('span span').html('(0)');
			if (ajax_translations.hide_empty == "Yes") {jQuery(this).parent().css('display', 'none');}
		}
		else {jQuery(this).parent().css('display', 'block').find('span span').html('(' + Sidebar_Counts['Profuses'][ID] + ')');}
	});
